	...
	function parse() {
		...
		for (var i = startRow; i < params.length; i++) {
			...
			if (inRepeater && currentRow.indexOf('</nk-repeat') > -1) {
				var currentRepeaterTemplateWithIndexator = currentRepeaterTemplate.split('<nk-model>' +
					currentIndexatorName + '</nk-model>').join(currentCollection[currentIndex++]);
				...
				for (var k = 1; k < currentCollection.length; k++) {
					currentRepeaterTemplateWithIndexator = currentRepeaterTemplate.split('<nk-model>' +
						currentIndexatorName + '</nk-model>').join(currentCollection[currentIndex++]);
			...
			for (var j = 0; j < currentRow.length; j++) {
				...
				if (inNikolAngular && j + 14 < currentRow.length && currentRow.substr(j, 15) == 'template name="' &&
					!inModel) {
					inTemplate = true;
					newLine = false;
				...
				if (inNikolAngular && j + 16 < currentRow.length && currentRow.substr(j, 17) == 'template render="' &&
					!inModel) {
				...
				if (inNikolAngular && j + 11 < currentRow.length && currentRow.substr(j, 12) == 'if condition' &&
					currentRow.indexOf('>') > -1 && !inModel) {
				...
				if (inNikolAngular && j + 9 < currentRow.length && currentRow.substr(j, 10) == 'repeat for' &&
					currentRow.indexOf('>') > -1 && !inModel) {
